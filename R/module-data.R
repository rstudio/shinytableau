...
#'   restore.)
#' @param setting Either `NULL`, or a data spec object, as generated by
...
#'
#' @export
choose_data_unpack <- function(id, spec = isolate(tableau_setting(id))) {
  if (is.null(spec)) {
    return(NULL)
  }
  ...
  results[[ns(NS("agg", "choice"))]] <- spec[["source"]]
  if (identical(spec[["source"]], "underlying")) {
    results[[ns(NS("underlying", "choice"))]] <- spec[["table"]]
  } else if (identical(spec[["source"]], "datasource")) {
    results[[ns(NS("datasource", "choice"))]] <- spec[["ds"]]
    results[[ns(NS("logical", "choice"))]] <- spec[["table"]]
  }
  results
}
